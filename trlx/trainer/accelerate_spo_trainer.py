...
from trlx.pipeline.offline_pipeline import AsyncPromptPipeline
from trlx.pipeline.spo_pipeline import SPORolloutStorage
...
@register_trainer
class AccelerateSPOTrainer(AccelerateRLTrainer):
    """SPO Accelerate Trainer
    Use few-shot prompt to make experience, and original prompt to train."""

    reward_fn: Callable[[List[str], List[str], List[str]], List[float]]
    tokenizer: AutoTokenizer

    def __init__(self, config: TRLConfig, **kwargs):
        """SPO Accelerate Trainer initialization
        ...
        self.store = SPORolloutStorage(self.tokenizer.pad_token_id, self.tokenizer.padding_side)
    ...
    def loss(self, batch: SPORLBatch) -> Tuple[float, Dict[str, Any]]:
        """Computes loss on a batch of data and returns statistics

        Args:
            batch: `SPORLBatch` Previous batch of episodes
    ...
    def add_prompt_pipeline(self, pipeline: AsyncPromptPipeline):
    ...
    def make_experience(self, num_rollouts: int = 1024, iter_count: int = 0):  # noqa:
        ...
        then appends SPOElements to trainer's `store`.
        ...
        while len(spo_rl_elements) < num_rollouts:
            stats = {}
            ...
            samples = self.generate(batch["rollout_input_ids"], batch["rollout_attention_mask"])
            stats["time/rollout_generate"] = time() - rollout_generate_time

            prompt_tensors = batch.input_ids
            rollout_prompt_tensors = batch["rollout_input_ids"]
            device = samples.device

            prompt_sizes = torch.tensor([prompt_tensors.shape[1]] * len(prompt_tensors), device=device)
            rollout_prompt_sizes = torch.tensor([rollout_prompt_tensors.shape[1]] * len(prompt_tensors), device=device)
            ...
            padded_rollout_prompts = self.accelerator.pad_across_processes(
                rollout_prompt_tensors, dim=1, pad_index=self.tokenizer.eos_token_id, pad_first=False
            )
            gathered_samples = self.accelerator.gather(padded_samples)
            gathered_prompts = self.accelerator.gather(padded_prompts)
            gathered_prompt_sizes = self.accelerator.gather(prompt_sizes)
            gathered_rollout_prompts = self.accelerator.gather(padded_rollout_prompts)
            gathered_rollout_prompt_sizes = self.accelerator.gather(rollout_prompt_sizes)
            metadata = gather_dict({k: v for k, v in batch.items() if 
                                    "input_ids" not in k and "attention_mask" not in k})

            if self.accelerator.is_main_process:
                all_str_samples, all_str_rollout_prompts, all_str_outputs = self.decode(
                    gathered_rollout_prompts, gathered_samples, gathered_prompt_sizes, append_eos_token=True
                )
                ...
                all_scores = self.reward_fn(
                    ...
                    prompts=all_str_rollout_prompts,
            ...
            str_samples, str_rollout_prompts, str_outputs = self.decode(rollout_prompt_tensors, samples, append_eos_token=True)
            ...
            for sample_idx in range(n_samples):
                ...
                spo_rl_elements.append(
                    SPORLElement(
                        rollout_query_tensor=rollout_prompt_tensors[sample_idx],
            ...
            tbar.set_description(f"[rollout {len(spo_rl_elements)} / {num_rollouts}]")
        ...
        self.push_to_store(spo_rl_elements)
